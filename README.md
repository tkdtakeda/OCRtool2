...
アクセスしてください）。終了するにはターミナルで `Ctrl+C` を押します。
...
- `index.html` / `*.js` / `*.css`（このフォルダ直下） — 画面・設定・操作。
  帳票レイアウト等はこれまで通りブラウザのIndexedDBに保存されます。
- `server/` — Pythonサーバー本体。`matcher.py`（画像マッチング）・
  `processor.py`（傾き補正・罫線除去）・`ocr.py`（OCR）が、対応する
  `matcher_engine.js` / `processor.js` / `ocr.js` の処理をそのまま
  引き継いでいます（アルゴリズム・設定項目とも変更していません）。
